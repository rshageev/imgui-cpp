...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    std::string value_text = ImFormatStringToStringV(fmt, args);
    const char* value_text_begin = value_text.data();
    const char* value_text_end = value_text.data() + value_text.size();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImCol col = GetColor((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImCol bg_col = GetColor((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    const ImCol text_col = GetColor(ImGuiCol_Text);
    RenderNavHighlight(bb, id);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    const ImCol col = GetColor(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered);
    ...
    ImCol cross_col = GetColor(ImGuiCol_Text);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // Render
    ImCol bg_col = GetColor((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    ImCol text_col = GetColor(ImGuiCol_Text);
    if (hovered || held) {
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(-ImClamp(std::floor((bb_frame_width - 2.0f) * 0.5f), 0.0f, 3.0f), -ImClamp(std::floor((bb_frame_height - 2.0f) * 0.5f), 0.0f, 3.0f)));
    ...
    // Render
    const ImCol bg_col = GetColor(ImGuiCol_ScrollbarBg);
    const ImCol grab_col = GetColor(held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab, alpha);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImColorf& tint_col, const ImColorf& border_col)
{
    ...
    {
        window->DrawList->AddImage(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColor(tint_col));
    }
}

// ImageButton() is flawed as 'id' is always derived from 'texture_id' (see #2464 #1390)
// We provide this internal helper to write your own variant while we figure out how to redesign the public ImageButton() API.
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImColorf& bg_col, const ImColorf& tint_col, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (bg_col.a > 0.0f) {
        window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding, GetColor(bg_col));
    }
    window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColor(tint_col));

    return pressed;
}

bool ImGui::ImageButton(const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImColorf& bg_col, const ImColorf& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const auto frame_col = GetColor((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderFrame(check_bb.Min, check_bb.Max, frame_col, true, style.FrameRounding);
    const ImCol check_col = GetColor(ImGuiCol_CheckMark);
    bool mixed_value = (g.LastItemData.InFlags & ImGuiItemFlags_MixedValue) != 0;
    if (mixed_value)
    {
        // Undocumented tristate/mixed/indeterminate checkbox (#2644)
        // This may seem awkwardly designed because the aim is to make ImGuiItemFlags_MixedValue supported by all widgets (not just checkbox)
        ImVec2 pad(ImMax(1.0f, std::floor(square_sz / 3.6f)), ImMax(1.0f, std::floor(square_sz / 3.6f)));
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    RenderNavHighlight(total_bb, id);
    window->DrawList->AddCircleFilled(center, radius, GetColor((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
    if (active)
    {
        const float pad = ImMax(1.0f, std::floor(square_sz / 6.0f));
        window->DrawList->AddCircleFilled(center, radius - pad, GetColor(ImGuiCol_CheckMark), 16);
    }

    if (style.FrameBorderSize > 0.0f)
    {
        window->DrawList->AddCircle(center + ImVec2(1, 1), radius, GetColor(ImGuiCol_BorderShadow), 16, style.FrameBorderSize);
        window->DrawList->AddCircle(center, radius, GetColor(ImGuiCol_Border), 16, style.FrameBorderSize);
    }
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    RenderFrame(bb.Min, bb.Max, GetColor(ImGuiCol_FrameBg), true, style.FrameRounding);
    bb.Expand(ImVec2(-style.FrameBorderSize, -style.FrameBorderSize));
    const ImVec2 fill_br = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
    RenderRectFilledRangeH(window->DrawList, bb, GetColor(ImGuiCol_PlotHistogram), 0.0f, fraction, style.FrameRounding);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        // Draw
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColor(ImGuiCol_Separator));
    }
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    const ImCol separator_col = GetColor(ImGuiCol_Separator);
    if (label_size.x > 0.0f)
    {
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay, ImCol bg_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    const ImCol col = GetColor(held ? ImGuiCol_SeparatorActive : (hovered && g.HoveredIdTimer >= hover_visibility_delay) ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Render shape
    const ImCol frame_col = GetColor(hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ...
    {
        const ImCol bg_col = GetColor((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        const ImCol text_col = GetColor(ImGuiCol_Text);
...
static_assert(IM_ARRAYSIZE(GDataTypeInfo) == ImGuiDataType_COUNT);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    const ImCol frame_col = GetColor(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        FLOATTYPE v_min_fudged = (std::abs((FLOATTYPE)v_min) < logarithmic_zero_epsilon) ? ((v_min < 0.0f) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (FLOATTYPE)v_min;
        FLOATTYPE v_max_fudged = (std::abs((FLOATTYPE)v_max) < logarithmic_zero_epsilon) ? ((v_max < 0.0f) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (FLOATTYPE)v_max;
        ...
        else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
        {
            ...
            if (v == 0.0f)
                ...
                result = (1.0f - (float)(std::log(-(FLOATTYPE)v_clamped / logarithmic_zero_epsilon) / std::log(-v_min_fudged / logarithmic_zero_epsilon))) * zero_point_snap_L;
            else
                result = zero_point_snap_R + ((float)(std::log((FLOATTYPE)v_clamped / logarithmic_zero_epsilon) / std::log(v_max_fudged / logarithmic_zero_epsilon)) * (1.0f - zero_point_snap_R));
        }
        else if ((v_min < 0.0f) || (v_max < 0.0f)) // Entirely negative slider
            result = 1.0f - (float)(std::log(-(FLOATTYPE)v_clamped / -v_max_fudged) / std::log(-v_min_fudged / -v_max_fudged));
        else
            result = (float)(std::log((FLOATTYPE)v_clamped / v_min_fudged) / std::log(v_max_fudged / v_min_fudged));
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        {
            float zero_point_center = (-(float)ImMin(v_min, v_max)) / std::abs((float)v_max - (float)v_min); // The zero point in parametric space
            float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
            float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColor(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        if (p_clamp_min || p_clamp_max)
        {
            if (p_clamp_min && p_clamp_max && DataTypeCompare(data_type, p_clamp_min, p_clamp_max) > 0)
                std::swap(p_clamp_min, p_clamp_max);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        state->CurLenA = (int)(buf_end - buf);      // Here we'll cut off malformed UTF-8.
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (hovered && io.MouseClickedCount[0] >= 2 && !io.KeyShift)
        {
            ...
            else
            {
                ...
                {
                    std::swap(state->Stb.select_start, state->Stb.select_end);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_cut || is_copy)
        {
            ...
            if (io.SetClipboardTextFn)
            {
                ...
                auto clipboard_data = ImTextStrToUtf8(state->TextW.Data + ib, state->TextW.Data + ie);
                SetClipboardText(clipboard_data.c_str());
            }
            if (is_cut)
            {
                if (!state->HasSelection())
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                for (const char* s = clipboard; *s != 0; )
                {
                    ...
                    unsigned int c1 = c;
                    if (!InputTextFilterCharacter(&c1, flags, callback, callback_user_data, ImGuiInputSource_Clipboard))
                        continue;
                    clipboard_filtered[clipboard_filtered_len++] = (char32_t)c1;
                }
    ...
    if (g.ActiveId == id)
    {
        ...
        if (!is_readonly)
        {
            ...
            state->TextA = ImTextStrToUtf8(state->TextW.Data, NULL);
        }
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColor(ImGuiCol_FrameBg), true, style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if (cursor_offset.x < state->ScrollX)
                    state->ScrollX = std::floor(ImMax(0.0f, cursor_offset.x - scroll_increment_x));
                else if (cursor_offset.x - visible_width >= state->ScrollX)
                    state->ScrollX = std::floor(cursor_offset.x - visible_width + scroll_increment_x);
            }
            else
            {
                state->ScrollX = 0.0f;
        ...
        if (render_selection)
        {
            ...
            const ImCol bg_color = GetColor(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.0f) rect_size.x = std::floor(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
        ...
        {
            ImCol col = GetColor(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColor(ImGuiCol_Text));
            }
        }
    }
    else
    ...
    if (BeginChild("undopoints", ImVec2(0.0f, GetTextLineHeight() * 15), true)) // Visualize undo state
    {
        ...
        for (int n = 0; n < STB_TEXTEDIT_UNDOSTATECOUNT; n++)
        {
            ...
            if (undo_rec_type != ' ' && undo_rec->char_storage != -1) {
                buf = ImTextStrToUtf8(undo_state->undo_char + undo_rec->char_storage, undo_state->undo_char + undo_rec->char_storage + undo_rec->insert_length);
            }
...
static void ColorEditRestoreH(const float* col, float* H)
{
    ...
    if (g.ColorEditSavedID != g.ColorEditCurrentID || g.ColorEditSavedColor != ImGui::ColorConvertToByte(ImColorf(col[0], col[1], col[2], 0)))
        return;
...
static void ColorEditRestoreHS(const float* col, float* H, float* S, float* V)
{
    ...
    if (g.ColorEditSavedID != g.ColorEditCurrentID
        || g.ColorEditSavedColor != ImGui::ColorConvertToByte(ImColorf(col[0], col[1], col[2], 0)))
    {
        return;
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const float w_item_one  = ImMax(1.0f, std::floor((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
        const float w_item_last = ImMax(1.0f, std::floor(w_inputs - (w_item_one + style.ItemInnerSpacing.x) * (components - 1)));
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        const ImColorf col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                value_changed |= ColorPicker4("##picker", col, picker_flags, &g.ColorPickerRef.r);
            }
            EndPopup();
        }
    }
    ...
    if (value_changed && picker_active_window == NULL)
    {
        ...
        if ((flags & ImGuiColorEditFlags_DisplayHSV) && (flags & ImGuiColorEditFlags_InputRGB))
        {
            ...
            g.ColorEditSavedColor = ColorConvertToByte(ImColorf(f[0], f[1], f[2], 0));
        }
...
static void RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha)
{
    ...
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + half_sz.x + 1,         pos.y), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Right, ImCol(0,0,0,alpha8));
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + half_sz.x,             pos.y), half_sz,                              ImGuiDir_Right, ImCol(255,255,255,alpha8));
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + bar_w - half_sz.x - 1, pos.y), ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Left,  ImCol(0,0,0,alpha8));
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + bar_w - half_sz.x,     pos.y), half_sz,                              ImGuiDir_Left,  ImCol(255,255,255,alpha8));
}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bars_triangles_half_sz = std::floor(bars_width * 0.20f);
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            float cos_hue_angle = std::cos(-H * 2.0f * IM_PI);
            float sin_hue_angle = std::sin(-H * 2.0f * IM_PI);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ImColorf col_v4(col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
        if ((flags & ImGuiColorEditFlags_NoLabel))
            Text("Current");
        ...
        {
            Text("Original");
            ImColorf ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            g.ColorEditSavedID = g.ColorEditCurrentID;
            g.ColorEditSavedColor = ColorConvertToByte(ImColorf(col[0], col[1], col[2], 0));
        }
        else if (flags & ImGuiColorEditFlags_InputHSV)
        {
            col[0] = H;
    ...
    const auto style_alpha8 = ColorComponentToByte(style.Alpha);
    const ImCol col_black = ImCol(0,0,0,style_alpha8);
    const ImCol col_white = ImCol(255,255,255,style_alpha8);
    const ImCol col_midgrey = ImCol(128,128,128,style_alpha8);
    const ImCol col_hues[6 + 1] = {
        ImCol(255,0,0,style_alpha8),
        ImCol(255,255,0,style_alpha8),
        ImCol(0,255,0,style_alpha8),
        ImCol(0,255,255,style_alpha8),
        ImCol(0,0,255,style_alpha8),
        ImCol(255,0,255,style_alpha8),
        ImCol(255,0,0,style_alpha8)
    };

    ImColorf hue_color_f(1, 1, 1, style.Alpha);
    ColorConvertHSVtoRGB(H, 1, 1, hue_color_f.r, hue_color_f.g, hue_color_f.b);
    ImCol hue_color32 = ColorConvertToByte(hue_color_f);
    ImCol user_col32_striped_of_alpha = ColorConvertToByte(ImColorf(R, G, B, style.Alpha)); // Important: this is still including the main rendering/style alpha!!
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            ImVec2 gradient_p0(wheel_center.x + std::cos(a0) * wheel_r_inner, wheel_center.y + std::sin(a0) * wheel_r_inner);
            ImVec2 gradient_p1(wheel_center.x + std::cos(a1) * wheel_r_inner, wheel_center.y + std::sin(a1) * wheel_r_inner);
        ...
        const ImDrawVert vertices[] = {
            // colored gradient triangle
            {tra, uv_white, ImCol::ToU32(hue_color32)},
            {trb, uv_white, ImCol::ToU32(hue_color32)},
            {trc, uv_white, ImCol::ToU32(col_white)},
            // black to transparent gradient triangle
            {tra, uv_white, 0},
            {trb, uv_white, ImCol::ToU32(col_black)},
            {trc, uv_white, 0},
        };
        draw_list->AddGeometry(vertices);     
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        sv_cursor_pos.x = ImClamp(std::round(picker_pos.x + ImSaturate(S)     * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the circle to stick out too much
        sv_cursor_pos.y = ImClamp(std::round(picker_pos.y + ImSaturate(1 - V) * sv_picker_size), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2);
        ...
        float bar0_line_y = std::round(picker_pos.y + H * sv_picker_size);
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min, bar1_bb.Max, ImCol::BlackTransp, bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f));
        draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max,
            user_col32_striped_of_alpha,
            user_col32_striped_of_alpha,
            user_col32_striped_of_alpha.WithAlpha(0),
            user_col32_striped_of_alpha.WithAlpha(0));
        float bar1_line_y = std::round(picker_pos.y + (1.0f - alpha) * sv_picker_size);
...
bool ImGui::ColorButton(const char* desc_id, const ImColorf& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImColorf col_rgb = col;
    if (flags & ImGuiColorEditFlags_InputHSV) {
        ColorConvertHSVtoRGB(col_rgb.r, col_rgb.g, col_rgb.b, col_rgb.r, col_rgb.g, col_rgb.b);
    }

    ImColorf col_rgb_without_alpha(col_rgb.r, col_rgb.g, col_rgb.b, 1.0f);
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.a < 1.0f)
    {
        float mid_x = std::round((bb_inner.Min.x + bb_inner.Max.x) * 0.5f);
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColor(col_rgb), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawFlags_RoundCornersRight);
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColor(col_rgb_without_alpha), rounding, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
        // Because GetColor() multiplies by the global style Alpha and we don't want to display a checkerboard if the source code had no alpha
        ImColorf col_source = (flags & ImGuiColorEditFlags_AlphaPreview) ? col_rgb : col_rgb_without_alpha;
        if (col_source.a < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColor(col_source), grid_step, ImVec2(off, off), rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColor(col_source), rounding);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
    ...
    if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered) {
        ColorTooltip(desc_id, &col.r, flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf));
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ImColorf cf(col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
...
bool ImGui::TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
{
    ...
    std::string label = ImFormatStringToStringV(fmt, args);
    return TreeNodeBehavior(window->GetID(str_id), flags, label.data(), label.data() + label.size());
}
...
bool ImGui::TreeNodeExV(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
{
    ...
    std::string label = ImFormatStringToStringV(fmt, args);
    return TreeNodeBehavior(window->GetID(ptr_id), flags, label.data(), label.data() + label.size());
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        frame_bb.Min.x -= std::floor(window->WindowPadding.x * 0.5f - 1.0f);
        frame_bb.Max.x += std::floor(window->WindowPadding.x * 0.5f);
    }
    ...
    {
        // Framed type
        const ImCol bg_col = GetColor((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (span_all_columns && g.CurrentTable)
    ...
    if (hovered || selected)
    {
        const ImCol col = GetColor((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        const ImCol col_base = GetColor((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines : ImGuiCol_PlotHistogram);
        const ImCol col_hovered = GetColor((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered : ImGuiCol_PlotHistogramHovered);
...
bool ImGui::BeginMenuBar()
{
    ...
    ImRect clip_rect(
        std::round(bar_rect.Min.x + window->WindowBorderSize),
        std::round(bar_rect.Min.y + window->WindowBorderSize),
        std::round(ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->WindowBorderSize))),
        std::round(bar_rect.Max.y));
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        popup_pos = ImVec2(pos.x - 1.0f - std::floor(style.ItemSpacing.x * 0.5f), pos.y - style.FramePadding.y + window->MenuBarHeight());
        window->DC.CursorPos.x += std::floor(style.ItemSpacing.x * 0.5f);
        ...
        PopStyleVar();
        window->DC.CursorPos.x += std::floor(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        ...
        float checkmark_w = std::floor(g.FontSize * 1.20f);
        ...
        RenderArrow(window->DrawList, pos + ImVec2(offsets->OffsetMark + extra_w + g.FontSize * 0.30f, 0.0f), GetColor(ImGuiCol_Text), ImGuiDir_Right);
    }
    if (!enabled)
        EndDisabled();
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL)
        {
            ...
            float extra = ImClamp(std::abs(ta.x - tb.x) * 0.30f, ref_unit * 0.5f, ref_unit * 2.5f);   // add a bit of extra slack.
            ta.x += child_dir * -0.5f;
...
void ImGui::EndMenu()
{
    ...
    if (window->BeginCount == window->BeginCountPreviousFrame) {
        if (g.NavMoveDir == ImGuiDir_Left && NavMoveRequestButNoResultYet()) {
            if (g.NavWindow
                && (g.NavWindow->RootWindowForNav == window)
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    // Draw separator
    const ImCol col = GetColor((flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive);
    const float y = tab_bar->BarRect.Max.y - 1.0f;
    {
        const float separator_min_x = tab_bar->BarRect.Min.x - std::floor(window->WindowPadding.x * 0.5f);
        const float separator_max_x = tab_bar->BarRect.Max.x + std::floor(window->WindowPadding.x * 0.5f);
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget)
    {
        ...
        tab_bar->ScrollingSpeed = ImMax(tab_bar->ScrollingSpeed, std::abs(tab_bar->ScrollingTarget - tab_bar->ScrollingAnim) / 0.3f);
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (is_central_section)
        window->DC.CursorPos = tab_bar->BarRect.Min + ImVec2(std::floor(tab->Offset - tab_bar->ScrollingAnim), 0.0f);
    else
    ...
    const ImCol tab_col = GetColor((held || hovered) ? ImGuiCol_TabHovered : tab_contents_visible ? (tab_bar_focused ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive) : (tab_bar_focused ? ImGuiCol_Tab : ImGuiCol_TabUnfocused));
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImCol col)
{
    ...
    if (g.Style.TabBorderSize > 0.0f)
    {
        ...
        draw_list->PathStroke(GetColor(ImGuiCol_Border), 0, g.Style.TabBorderSize);
    }
}
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    else if (unsaved_marker_visible)
    {
        ...
        RenderBullet(draw_list, bullet_bb.GetCenter(), GetColor(ImGuiCol_Text));
    }
